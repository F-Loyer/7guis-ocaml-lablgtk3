(*
List.iter (fun (col,row, expr) ->
  Printf.printf "%d,%d: " col row;
  let e = Parser.prog Lexer.read (Lexing.from_string expr) in
  Expr.print_expr e;
  Expr.add_formulae (col,row) e;
  Printf.printf "===\n%!";
  let cell = Hashtbl.find Expr.spreadsheet (col,row) in
  print_string (Expr.string_of_value cell.value);
  print_string "\n"
) [
  0,0, "B0+B1";
  1,0, "1";
...
let _ = listview#enable_grid_lines
...
let _ = listview#append_column indice_colview
...
let colviews = Array.mapi (fun i renderer -> GTree.view_column ~title:colnames.(i) ~renderer ()) cell_renderers
let _ = Array.iter (fun colview -> ignore @@ listview#append_column colview) colviews

let selection = listview#selection 
let _ = selection#set_mode `NONE

let search_col s =
  Char.code (s.[0]) - (Char.code 'A')

let _ = listview#connect#row_activated ~callback:(fun p c ->
  if String.length c#title = 1 then
  begin
    let col = search_col c#title in
    let row = (GTree.Path.get_indices p).(0) in
    model#set ~row:(model#get_iter p) ~column:table_cols.(col) (Expr.get_formulae (col,row));
    listview#set_cursor ~edit:true p c
  end)
...
let () = for col=0 to 25 do
  ignore @@ (fst cell_renderers.(col))#connect#edited ~callback:(fun p str -> 
    let row = (GTree.Path.get_indices p).(0) in
    try
      if (String.length str) >0 then
        if str.[0] = '=' ||  str.[0] >='0' && str.[0] <='9' then
    ...
    _ ->
      Expr.add_formulae (col,row) str (Expr.Invalid "Parse error");
       model#set ~row:(model#get_iter p) ~column:table_cols.(col) "Invalid")
  done
let () = ignore (indice_colview)
