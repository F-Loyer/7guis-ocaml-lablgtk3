...
let label1 = GMisc.label ~packing:(hbox1#pack ~padding:4) ~text:"Elapsed time:" ()
...
let label2 = GMisc.label ~packing:(hbox2#pack ~padding:4) ~text:"Duration:" ()
...
let start_timer () = timer := Some (GMain.Timeout.add ~ms:100 ~callback:(fun () -> 
  t := !t +. 0.1; update ();
  true))
...
let _ = button#connect#clicked ~callback:(fun _ -> reset ())
let _ = slider#connect#value_changed ~callback:(fun _ -> update ())
let () = reset ()

let _ = ignore (label1, label2)
